"""Coverage/lag alarm, storage sampling, and integrity scanning. See plan:
Coverage / lag alarm, and the `storage_sample`/`integrity_scan` rows in
Scheduling.
...
trending toward data loss" and "notice bit rot before Pluton's next
...
# physically on the card (see plan: "each run records a device_storage_samples
# row (from storage plus the earliest recording seen in a full search)"). The
# card's own loop-overwrite bounds what actually comes back regardless of how
# wide this is; ~10 years comfortably exceeds any real SD card's retention.
...
def record_storage_sample(
    *, repository: Repository, provider: CameraProvider, device_id: int
    ...
    """The `storage_sample` row in Scheduling: runs "each run" (scheduled,
    backfill, or manual), not on its own schedule."""
...
def coverage_margin(
    *, repository: Repository, device_id: int, margin: float = DEFAULT_COVERAGE_MARGIN
    ...
    """See plan: Coverage / lag alarm. Alarms when the oldest recording
    ReoVault hasn't archived yet is within `margin` (default 20%) of the
    card's own loop-overwrite horizon, i.e. the safety buffer before that
    recording gets overwritten unarchived is running out."""
...
def run_integrity_scan(
    *,
    ...
    """See plan: `integrity_scan` ("re-reads a rotating sample... re-verifying
    GCM tags and SHA-256, so bit rot surfaces while Pluton still holds a good
    copy"). `bucket_index` defaults to the day of the year, so repeated
    weekly calls rotate through the full archived set over time without a
    persisted cursor."""
