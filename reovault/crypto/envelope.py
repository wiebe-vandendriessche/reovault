"""Per-file envelope encryption. See plan: Encryption > Format.
...
range map arithmetically onto a frame range with no index (see plan:
Dashboard streaming), which is exactly what `read_range` below does.
...
def read_range(src: SeekableReadableStream, master_key: bytes, offset: int, length: int) -> bytes:
    ...
    (see plan) so seeking a two-hour recording never decrypts more than the
    frames touched. `src` must be seekable."""
    if length <= 0:
        return b""
    header = _read_header(src)
    dek = unwrap_dek(header.dek_nonce, header.wrapped_dek, master_key)
