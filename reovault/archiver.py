"""The archive state machine: the core of the system. See plan: Reliability.
...
# to `quarantined` (needs a human). See plan: Failure classification. Input
# and protocol errors have no entry: they quarantine on the first failure.
...
class AlreadyRunningError(Exception):
    """Another `run()`/`reconcile()` holds the single-instance lock (see
...
def compute_backoff(attempts: int) -> timedelta:
    """Exponential backoff capped at 6h with ±25% jitter (see plan: Failure
    classification). `attempts` is the attempt number just made (1-indexed):
    attempt 1 -> ~1m, attempt 2 -> ~2m, attempt 3 -> ~4m, ..."""
...
def assert_same_filesystem(staging_dir: Path, vault_dir: Path) -> None:
    """See plan: Startup preflight. Atomic rename (state-machine step 6)
    requires the `.tmp` file and its final destination to be on the same
    filesystem; failing loudly here beats failing mid-archive at 05:00."""
...
def assert_pinned_cli_version(binary: str, pinned_version: str) -> None:
    """See plan: Startup preflight and "pin and validate external tool
    versions when behavior affects correctness" (CLAUDE.md)."""
    try:
...
def _parse_vault_path(vault_path: str) -> tuple[str, datetime, str] | None:
    ...
    shaped like `<alias>/<YYYY>/<MM>/<DD>/<start>_<remote_name>.enc` (see
    plan: Vault layout). None if it doesn't match that shape at all, in
    which case reconciliation can't attribute it to any recording."""
...
class Archiver:
    ...
    def _run_locked(
        ...
        # See plan: Time handling. Widening absorbs DST ambiguity; duplicates
        # from the overlap cost nothing, the unique index absorbs them.
    ...
    def _handle_provider_failure(
        ...
        if exc.error_class == ErrorClass.AUTH:
            # Per plan: stop the run immediately. Retrying a bad password can
            # lock the device account, so this recording gets one more
            # chance next run, not a tight retry loop right now.
            self.repository.mark_failed(
                row_id, error=str(exc), error_class=exc.error_class, next_attempt_at=None
            )
            raise _AbortRun(str(exc)) from exc
        ...
        if exc.error_class == ErrorClass.INPUT or exc.error_class == ErrorClass.PROTOCOL:
            ...
            self.repository.mark_quarantined(row_id, error=str(exc), error_class=exc.error_class)
            return
        ...
        if attempts >= max_attempts:
            ...
            self.repository.mark_failed(
                row_id, error=str(exc), error_class=exc.error_class, next_attempt_at=next_attempt_at
            )
    ...
    # -- reconciliation -----------------------------------------------------
    # See plan: Reliability. Sweeps orphaned staging files (a crash between
    # steps 2 and 8) and adopts/deletes unreferenced vault files (a crash
    # between steps 6 and 7). Takes the same lock as `run()`: nothing here is
    # safe to do while a run is actually in flight.
