...
def _rec(name: str, start: datetime, size: int):
    return make_recording(name, start, size=size, raw_metadata={"name": name})
...
def test_local_error_during_fetch_aborts_without_mutating_state(env):
    ...
    # not the recording's, so state must not advance on a storage failure.
...
def test_request_cancel_stops_before_the_next_recording(env):
    """Cooperative, not immediate (see the dashboard's Stop button, which
    calls this): the flag is only ever checked *between* recordings, so a
    fetch already in flight when Stop is clicked always finishes on its
    own -- clip1 here still ends up fully archived. clip2 is the one that
    never starts: cancel is set from inside clip1's own fetch, simulating
    a real click landing mid-run, and the loop must see it before touching
    clip2 at all (not even a `discovered` row for it)."""
    rec1 = _rec("clip1", datetime(2026, 4, 17, 12, tzinfo=UTC), 10)
    rec2 = _rec("clip2", datetime(2026, 4, 17, 13, tzinfo=UTC), 10)
    archiver_holder: list = []

    def _cancel_mid_fetch():
        archiver_holder[0].request_cancel()
        return None  # let this fetch succeed normally

    provider = FakeProvider(
        recordings=[
            ScriptedRecording(rec1, b"0123456789", on_fetch=_cancel_mid_fetch),
            ScriptedRecording(rec2, b"0123456789"),
        ]
    )
    archiver = env.archiver(provider)
    archiver_holder.append(archiver)

    result = archiver.run(trigger="manual", from_utc=WINDOW_FROM, to_utc=WINDOW_TO)

    assert result.downloaded == 1
    rows = env.repository.conn.execute("SELECT remote_name, state FROM recordings").fetchall()
    assert {r["remote_name"]: r["state"] for r in rows} == {"clip1": "archived"}

    run_row = env.repository.conn.execute("SELECT * FROM archive_runs").fetchone()
    assert run_row["outcome"] == "aborted"
    assert run_row["error"] == "canceled"


def test_cancel_flag_is_cleared_at_the_start_of_a_new_run(env):
    """A canceled run must not poison the *next* run on the same Archiver
    instance (the dashboard reuses one Archiver per device across every
    scheduled/manual/backfill job)."""
    rec = _rec("clip1", datetime(2026, 4, 17, 12, tzinfo=UTC), 10)
    provider = FakeProvider(recordings=[ScriptedRecording(rec, b"0123456789")])
    archiver = env.archiver(provider)
